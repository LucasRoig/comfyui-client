  ...
  public execute(input: z.infer<typeof dtoSchema>) {
    return DbUtils.executeAndExpectDefined(
      ...
      .andThen(({ insertFieldsPayload: _, insertFieldsResult, ...rest }) => {
        ...
        const insertStringFieldsPayload = rest.fields.stringFields
          .map((f) => ({
            ...f,
            id: insertedIdByFieldId.get(f.fieldId),
          }))
          .filter((f): f is Omit<typeof f, "id"> & { id: string } => f.id !== undefined);
        const insertInputImageFieldsPayload = rest.fields.inputImageFields
          .map((f) => ({
            ...f,
            id: insertedIdByFieldId.get(f.fieldId),
          }))
          .filter((f): f is Omit<typeof f, "id"> & { id: string } => f.id !== undefined);
        const insertOutputImageFieldsPayload = rest.fields.outputImageFields
          .map((f) => ({
            ...f,
            id: insertedIdByFieldId.get(f.fieldId),
          }))
      ...
      .andThrough(({ insertStringFieldsPayload }) =>
        DbUtils.executeIf(insertStringFieldsPayload.length > 0, () =>
      ...
      .andThrough(({ insertInputImageFieldsPayload }) =>
        DbUtils.executeIf(insertInputImageFieldsPayload.length > 0, () =>
      ...
      .andThrough(({ insertOutputImageFieldsPayload }) =>
        DbUtils.executeIf(insertOutputImageFieldsPayload.length > 0, () =>
